...
def convert_to_sequences(data_scaled, sequence_length):
    try:
        X = [data_scaled[i:i + sequence_length] for i in range(len(data_scaled) - sequence_length)]
...
def main():
    try:
        ...
        X = np.concatenate([convert_to_sequences(d, sequence_length)[0] for d in data_scaled])
        y = np.concatenate([convert_to_sequences(d, sequence_length)[1] for d in data_scaled])
